...
import ctypes
import numpy as np
clib = ctypes.cdll.LoadLibrary("./libcldpcd.so")
...
cdecode = clib.ldpc_decode
cdecode.argtypes = [dbl_ptype, u8_ptype]
...
for repeat in range(500):
    msg = np.unpackbits(np.random.randint(0, 256, 16).astype(np.uint8))
    cw = np.dot(msg, G) % 2
    llr = -1 * ((2 * cw.astype(np.double)) - 1) + np.random.randn(256) * sigma
    result = np.empty(32, np.uint8)
    t0 = time.time()
    cdecode(llr, result)
    t1 = time.time()
    times.append(t1 - t0)
print("Avg time, snr {}, 500 msgs, C   :".format(snr), np.mean(times))
...
for repeat in range(500):
    ...
    t1 = time.time()
    times.append(t1 - t0)
print("Avg time, snr {}, 500 msgs, Rust:".format(snr), np.mean(times))
